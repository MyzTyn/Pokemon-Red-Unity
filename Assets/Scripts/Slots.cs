...
public class Slots : MonoBehaviour
{
    ...
    void UpdatePositions(bool addHalf)
    {
        if (canroll)
        {
            ...
            row1.transform.localPosition =
                new Vector3(row1.transform.localPosition.x, -152 + row1Index * 16 - row1Half * 8, 0);
            row2.transform.localPosition =
                new Vector3(row2.transform.localPosition.x, -152 + row2Index * 16 - row2Half * 8, 0);
            row3.transform.localPosition =
                new Vector3(row3.transform.localPosition.x, -152 + row3Index * 16 - row3Half * 8, 0);
    ...
    IEnumerator LinedUp()
    {
        ...
        if (CurrentMode != "SUPER" && (whatwaslinedup == "7" || whatwaslinedup == "BAR"))
        {
            //if the current mode isn't Super and there's a bar or 7 match, move the 3rd wheel down until there isn't a 7 or bar match
        ...
        if (CurrentMode == "BAD" && whatwaslinedup != "")
        {
            //if the current mode is Bad and there's a match, move the 3rd wheel down until there isn't a match
    ...
    {
        return above1 == above2 || above1 == middle2 ? above1 :
            middle1 == middle2 ? middle1 :
            below1 == middle2 || below1 == below2 ? below1 : "";
    }

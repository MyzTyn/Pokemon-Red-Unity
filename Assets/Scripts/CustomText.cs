...
public class CustomText : MaskableGraphic
{
    ...
    [SerializeField] [TextArea(3, 10)] private string field;
    ...
    [SerializeField] private AlignmentType m_AlignmentType = AlignmentType.Left;
    ...
    {
        get { return fontAtlas.tex == null ? s_WhiteTexture : fontAtlas.tex; }
    }
    ...
    void UpdateText()
    {
        ...
        for (int i = 0; i < text.Length; i++)
        {
            ...
            {
                //is the current character a left bracket?
                foreach (BracketChar bracketChar in fontAtlas.bracketChars)
                {
                    if (text.Substring(i).IndexOf("<" + bracketChar.name + ">") == 0)
                    {
                        //did we find a bracket expression?
    ...
    protected override void OnPopulateMesh(VertexHelper vh)
    {
       ...
       */
        ...
        for (int i = 0; i < lines.Count; i++)
        {
            ...
            {
                FontChar currentChar = lines[i]
                    .characters[alignmentType == AlignmentType.Right ? lines[i].characters.Count - j - 1 : j];
                ...
                vh.AddVert(new Vector3(x + size.x, y), this.color,
                    new Vector2((uv.x + size.x) / texSize.x, uv.y / texSize.y));
                vh.AddVert(new Vector3(x + size.x, y + size.y), this.color,
                    new Vector2((uv.x + size.x) / texSize.x, (uv.y + size.y) / texSize.y));
                vh.AddVert(new Vector3(x, y + size.y), this.color,
                    new Vector2(uv.x / texSize.x, (uv.y + size.y) / texSize.y));

...
public class NPC : MonoBehaviour
{
    ...
    {
        if (movementDelay == 1)
            dontUpdateDireciton = true; //moonwalking bug if exiting npc dialogue the frame before moving
    ...
    public IEnumerator MoveNPC(Direction direction)
    {
        ...
        Vector3 delta = (direction == Direction.Up ? Vector3.up :
    ...
    void UpdateSprite()
    {
        ...
        else
        {
            ...
            {
                case Direction.Up:
                    topRenderer.sprite = Sprite.Create(moveUpSprites[0],
                        new Rect(0f, 64f - (float)frame / 4f * 64, 16, 16), new Vector2(0.5f, 0.5f), 16);
                    bottomRenderer.sprite = Sprite.Create(moveUpSprites[1],
                        new Rect(0f, 64f - (float)frame / 4f * 64, 16, 16), new Vector2(0.5f, 0.5f), 16);
                    break;
                case Direction.Down:
                    topRenderer.sprite = Sprite.Create(moveDownSprites[0],
                        new Rect(0f, 64f - (float)frame / 4f * 64, 16, 16), new Vector2(0.5f, 0.5f), 16);
                    bottomRenderer.sprite = Sprite.Create(moveDownSprites[1],
                        new Rect(0f, 64f - (float)frame / 4f * 64, 16, 16), new Vector2(0.5f, 0.5f), 16);
                    break;
                case Direction.Left:
                    topRenderer.sprite = Sprite.Create(moveLeftSprites[0],
                        new Rect(0f, 32f - (float)frame / 2f * 32, 16, 16), new Vector2(0.5f, 0.5f), 16);
                    bottomRenderer.sprite = Sprite.Create(moveLeftSprites[1],
                        new Rect(0f, 32f - (float)frame / 2f * 32, 16, 16), new Vector2(0.5f, 0.5f), 16);
                    break;
                case Direction.Right:
                    topRenderer.sprite = Sprite.Create(moveRightSprites[0],
                        new Rect(0f, 32f - (float)frame / 2f * 32, 16, 16), new Vector2(0.5f, 0.5f), 16);
                    bottomRenderer.sprite = Sprite.Create(moveRightSprites[1],
                        new Rect(0f, 32f - (float)frame / 2f * 32, 16, 16), new Vector2(0.5f, 0.5f), 16);
                    break;
            }
        }
    ...
    void CheckCollision()
    {
        ...
        if (!isMoving)
        {
            ...
            cannotMove[2] = tileToCheck.isWall || tileToCheck.isLedge || tileToCheck.isWater || objectExists[2] ||
                            !tileToCheck.hasTile;
            tileToCheck = new MapTile(new Vector3Int((int)transform.position.x + 1, (int)transform.position.y, 0));
            cannotMove[3] = tileToCheck.isWall || tileToCheck.isLedge || tileToCheck.isWater || objectExists[3] ||
                            !tileToCheck.hasTile;
            tileToCheck = new MapTile(new Vector3Int((int)transform.position.x, (int)transform.position.y + 1, 0));
            cannotMove[0] = tileToCheck.isWall || tileToCheck.isLedge || tileToCheck.isWater || objectExists[0] ||
                            !tileToCheck.hasTile;
            tileToCheck = new MapTile(new Vector3Int((int)transform.position.x, (int)transform.position.y - 1, 0));
            cannotMove[1] = tileToCheck.isWall || tileToCheck.isLedge || tileToCheck.isWater || objectExists[1] ||
                            !tileToCheck.hasTile;
        }
    ...
    public void CheckObjectCollision()
    {
        ...
        ray = Physics2D.Raycast(transform.position,
            (direction == Direction.Up ? Vector2.up :
                ...
                direction == Direction.Left ? Vector2.left : Vector2.right), 1, playerMask);

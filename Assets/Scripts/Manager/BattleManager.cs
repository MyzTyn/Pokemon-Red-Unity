...
public class BattleManager : MonoBehaviour
{
    ...
    public IEnumerator WildBattleStart()
    {
        ...
        while (initialTimer < 1.2f)
        {
            ...
            playerObject.transform.localPosition =
                Vector3.Lerp(new Vector3(188, 76, 0), new Vector3(38, 76, 0), initialTimer / 1.2f);
            enemyMonObject.transform.localPosition = Vector3.Lerp(new Vector3(-28, 116, 0), new Vector3(124, 116, 0),
                initialTimer / 1.2f);
            yield return new WaitForEndOfFrame();
        }
        ...
        while (initialTimer < 0.6f)
        {
            initialTimer += Time.deltaTime;
            playerObject.transform.localPosition =
                Vector3.Lerp(new Vector3(38, 76, 0), new Vector3(-26, 76, 0), initialTimer / 0.6f);
            yield return new WaitForEndOfFrame();
        }
    ...
    public void SetBackMonImageActive()
    {
        ...
        SoundManager.instance.PlayCry((int)playermon.Species - 1);
    ...
    void Update()
    {
        ...
        if (battleState == BattleState.PlayerTurn && Dialogue.instance.finishedText)
        {
            ...
            {
                cursor.SetPosition(48 * (selectedOption % 2) + 72,
                    -16 * Mathf.FloorToInt((float)selectedOption / 2f) + 24);
    ...
    void UpdateStatsUI()
    {
        ...
        playerHpText.text = (playermon.HP > 99 ? "" : playermon.HP > 9 ? " " : "  ") +
                            playermon.HP + " " + playermon.TotalHP;

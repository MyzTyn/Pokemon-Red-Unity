...
public class Pokemon
{
    ...
    public void RecalculateStats()
    {
        ...
        maxHP =
            Mathf.FloorToInt(((pokemonDataEntry.baseStats[0] + ivs[0]) * 2 + Mathf.Sqrt(evs[0]) / 4) * level / 100) +
            level + 10;
        attack =
            Mathf.FloorToInt(((pokemonDataEntry.baseStats[1] + ivs[1]) * 2 + Mathf.Sqrt(evs[1]) / 4) * level / 100) + 5;
        defense = Mathf.FloorToInt(
            ((pokemonDataEntry.baseStats[2] + ivs[2]) * 2 + Mathf.Sqrt(evs[2]) / 4) * level / 100) + 5;
        speed =
            Mathf.FloorToInt(((pokemonDataEntry.baseStats[3] + ivs[3]) * 2 + Mathf.Sqrt(evs[3]) / 4) * level / 100) + 5;
        special = Mathf.FloorToInt(
            ((pokemonDataEntry.baseStats[4] + ivs[4]) * 2 + Mathf.Sqrt(evs[4]) / 4) * level / 100) + 5;
        currentHP = maxHP;
    }
    ...
    public void GenerateIvs()
    {
        ...
        ivs[0] = (ivs[1] % 2 == 1 ? 8 : 0) + (ivs[2] % 2 == 1 ? 4 : 0) + (ivs[3] % 2 == 1 ? 2 : 0) +
                 (ivs[4] % 2 == 1 ? 1 : 0);
    }
    ...
    int CalculateLevelFromExp()
    {
        for (int i = 0; i <= 100; i++)
        {
            //iterate through each level to find the level from the current exp

...
public class Player : Singleton<Player>
{
    ...
    List<string> downLedgeSprites = new List<string>(new string[]
    {
        "Tileset_73", "Tileset_74", "Tileset_75", "Tileset_76", "Tileset_77", "Tileset_78", "Tileset_117",
        "Tileset_120", "Tileset_123"
    });
    ...
    IEnumerator MovementUpdate()
    {
            ...
            !manuallyWalking && !GameData.instance.atTitleScreen)
        {
            ...
            if (InputManager.Held(Button.Down) && !isDisabled && !ledgejumping && facedTile.hasTile &&
                facedTile.isLedge && downLedgeSprites.Contains(facedTile.tileName) && transform.position == targetPos &&
                direction == Direction.Down && holdFrames > 2)
            {
                ledgejumping = true;
                direction = Direction.Down;
            ...
            if (InputManager.Held(Button.Left) && !isDisabled && !ledgejumping && facedTile.hasTile &&
                facedTile.isLedge && leftLedgeSprites.Contains(facedTile.tileName) && transform.position == targetPos &&
                direction == Direction.Left && holdFrames > 2)
            {
                ledgejumping = true;
            ...
            if (InputManager.Held(Button.Right) && !isDisabled && !ledgejumping && facedTile.hasTile &&
                facedTile.isLedge && rightLedgeSprites.Contains(facedTile.tileName) &&
                transform.position == targetPos && direction == Direction.Right && holdFrames > 2)
            {
                ledgejumping = true;
            ...
            if (holdingDirection && !isWarping && onWarpTile && currentWarpTile.warpType == WarpType.WallWarp &&
                direction == currentWarpTile.wallDirection && transform.position == targetPos)
            {
            ...
            if (!ledgejumping)
            {
                if (InputManager.Held(Button.Up) || InputManager.Held(Button.Down) || InputManager.Held(Button.Left) ||
                    InputManager.Held(Button.Right))
                {
                    holdFrames++;
                    if (!holdingDirection)
                    {
                    ...
                    Direction inputDir = InputManager.Held(Button.Up) ? Direction.Up :
                        InputManager.Held(Button.Down) ? Direction.Down :
                        InputManager.Held(Button.Left) ? Direction.Left :
                        InputManager.Held(Button.Right) ? Direction.Right : 0;
                    ...
                    {
                        if (InputManager.Held(Button.Up) ? !cannotMoveUp :
                            InputManager.Held(Button.Down) ? !cannotMoveDown :
                            InputManager.Held(Button.Left) ? !cannotMoveLeft :
                            InputManager.Held(Button.Right) ? !cannotMoveRight : false)
                        {
                            if (walkSurfBikeState == MovementState.Surf && facedTile.hasTile && !facedTile.isWater &&
                                !facedTile.isWall && !facedTile.isLedge)
                            {
                                walkSurfBikeState = MovementState.Walk;
                                PlayCurrentAreaSong();
                            }

                            targetPos += InputManager.Held(Button.Up) ? Vector3.up :
                                InputManager.Held(Button.Down) ? Vector3.down :
                                InputManager.Held(Button.Left) ? Vector3.left :
                                InputManager.Held(Button.Right) ? Vector3.right : Vector3.zero;
                            isMoving = true;
                        }
                    }
                }
                ...
                if (transform.position == targetPos)
                {
                    if (isMoving) //did we move onto a new tile?
                    {
                        ...
                        if (!walkedfromwarp)
                            walkedfromwarp = true;
                        MapTile currentTile =
                            new MapTile(new Vector3Int((int)transform.position.x, (int)transform.position.y, 0));
                        if (currentTile.hasTile)
                        {
                            if (onWarpTile && currentWarpTile.warpType == WarpType.WalkOnWarp)
                            {
                                if (doorSprites.Contains(currentTile.tileName))
                                    SoundManager.instance.PlayGoInsideSound();
                                else SoundManager.instance.PlayGoOutsideSound();
                                DoWarp(currentWarpTile);
                            }

                            if (numberOfNoRandomBattleStepsLeft > 0) numberOfNoRandomBattleStepsLeft--;
                            if (((currentTile.hasGrass && currentAreaTable != null) ||
                                 (currentTile.isWater && areaHasWaterEncounters)) && !disableEncounters)
                            {
                ...
                if (InputManager.Held(Button.Up) || InputManager.Held(Button.Left) || InputManager.Held(Button.Right) ||
                    InputManager.Held(Button.Down)) holdingDirection = true;


                if (transform.position == targetPos) playerAnim.SetFloat("movedir", (int)direction + 1);
                collisionSoundTimer += Time.deltaTime;
                if (collisionSoundTimer >= 0.3f && (holdingFacingDirection() && facingWall()) && !ledgejumping &&
                    holdFrames > 2)
                {
    ...
    {
        return (direction == Direction.Up && cannotMoveUp) || (direction == Direction.Down && cannotMoveDown) ||
               (direction == Direction.Left && cannotMoveLeft) || (direction == Direction.Right && cannotMoveRight);
    }


    public bool holdingFacingDirection()
    {
        return (direction == Direction.Up && InputManager.Held(Button.Up)) ||
               (direction == Direction.Down && InputManager.Held(Button.Down)) ||
               (direction == Direction.Left && InputManager.Held(Button.Left)) ||
               (direction == Direction.Right && InputManager.Held(Button.Right));
    }


    public Vector3 DirectionToVector(Direction dir)
    {
        return (dir == Direction.Up ? Vector3.up :
            dir == Direction.Down ? Vector3.down :
            dir == Direction.Left ? Vector3.left :
            dir == Direction.Right ? Vector3.right : Vector3.zero);
    }
    ...
    void Update()
    {
        ...
        if (!isDisabled && !menuActive && !startMenuActive)
        {
            ...
            if (InputManager.Released(Button.Down) || InputManager.Released(Button.Right) ||
                InputManager.Released(Button.Left) || InputManager.Released(Button.Up))
            {
        ...
        if (facedObject != null)
        {
            ...
            {
                if (!holdingDirection && !isMoving && !isDisabled && Dialogue.instance.finishedText &&
                    !startMenuActive && !menuActive && !inBattle && !ledgejumping)
                {
    ...
    public void UpdateFacedTile()
    {
        ...
        tileToCheck = new MapTile(new Vector3Int((int)transform.position.x + (int)offset.x,
            (int)transform.position.y + (int)offset.y, 0));
    ...
    void CheckCollision()
    {
        ...
        cannotMoveLeft = tileToCheck.isWall || tileToCheck.isLedge ||
                         (tileToCheck.isWater && walkSurfBikeState != MovementState.Surf) || objectExists[2] ||
                         !tileToCheck.hasTile;
        tileToCheck = new MapTile(new Vector3Int((int)transform.position.x + 1, (int)transform.position.y, 0));
        cannotMoveRight = tileToCheck.isWall || tileToCheck.isLedge ||
                          (tileToCheck.isWater && walkSurfBikeState != MovementState.Surf) || objectExists[3] ||
                          !tileToCheck.hasTile;
        tileToCheck = new MapTile(new Vector3Int((int)transform.position.x, (int)transform.position.y + 1, 0));
        cannotMoveUp = tileToCheck.isWall || tileToCheck.isLedge ||
                       (tileToCheck.isWater && walkSurfBikeState != MovementState.Surf) || objectExists[0] ||
                       !tileToCheck.hasTile;
        tileToCheck = new MapTile(new Vector3Int((int)transform.position.x, (int)transform.position.y - 1, 0));
        cannotMoveDown = tileToCheck.isWall || tileToCheck.isLedge ||
                         (tileToCheck.isWater && walkSurfBikeState != MovementState.Surf) || objectExists[1] ||
                         !tileToCheck.hasTile;
    ...
    public void CheckObjectCollision()
    {
        ...
        RaycastHit2D ray = Physics2D.Raycast(transform.position,
            (direction == Direction.Up ? Vector2.up :
                ...
                direction == Direction.Left ? Vector2.left : Vector2.right), 1, layerMask);
        ...
        ray = Physics2D.Raycast(transform.position,
            (direction == Direction.Up ? Vector2.up :
                direction == Direction.Down ? Vector2.down :
                direction == Direction.Left ? Vector2.left : Vector2.right), 1, collisionMask);
    ...
    public void CheckMapCollision()
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            Vector2 dir = i == 0 ? Vector2.up :
                i == 1 ? Vector2.down :
                i == 2 ? Vector2.left :
                i == 3 ? Vector2.right : Vector2.zero;
            float rayDist = dir.y != 0 ? 5 : 6;

            // Debug.DrawLine(transform.position, transform.position + (Vector3)(dir * rayDist),Color.red,6f);
            hitColliders = Physics2D.BoxCastAll((Vector2)transform.position + new Vector2(0.5f, 0), new Vector2(11, 9),
                0, Vector2.zero);
        ...
        if (battleManager.battleType == BattleType.Wild)
        {
            ...
            switch (walkSurfBikeState)
            {
                case MovementState.Surf:
                    SoundManager.instance.FadeToSong(Music.Ocean);
                    break;
                case MovementState.Bike:
                    SoundManager.instance.FadeToSong(Music.Cycling);
                    break;
                default:
                    FadeToCurrentAreaSong();
                    break;
            }
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        if (col.gameObject.tag == "MapCollider")
        {
            ...
            if (forceBikeMaps.Contains(currentArea))
                walkSurfBikeState =
                    MovementState
                        .Bike; //if the map forces the player to use the bike, set the movement state to the bike

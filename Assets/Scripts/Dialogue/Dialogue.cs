    ...
    IEnumerator AnimateText(string strComplete)
    {
        ...
        if (currentDialogueType != DialogueType.Done)
        {
            ...
            while (i < strComplete.Length)
            {
                ...
                else if (i < strComplete.Length - 1 && strComplete[i] == '<')
                {
                    ...
                    foreach (BracketChar bracketChar in fontAtlas.bracketChars)
                    {
                        ...
                        if (strComplete.Substring(i).IndexOf(currentBracketChar) == 0)
                        {
                            //is the current bracket expression detected the current entry?

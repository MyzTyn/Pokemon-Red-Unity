        ...
        if (itemMode == Mode.Sell)
        {
            if (switchingItems)
            {
                selectCursor.anchoredPosition =
                    new Vector2(40, 104 - 16 * (selectBag - topItemIndex)) + new Vector2(4, 4);
        ...
        if (Dialogue.instance.finishedText)
        {
            if (currentMenu == Menu.QuantityMenu)
            {
                ...
                if (itemMode == Mode.Sell)
                {
                    ...
                    itemPrice = Kernal.ItemData[Inventory.instance.items[currentBagPosition].item].Price / 2;
            ...
            if (InputManager.Pressed(Button.A))
            {
                ...
                else if (currentMenu == Menu.ItemsWindow)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (!Inventory.instance.items[currentBagPosition].isKeyItem && Kernal
                                    .ItemData[Inventory.instance.items[currentBagPosition].item].Price > 0)
                            {
                                amountToTask = 1;
                                //UpdateQuantityScreen();
                                amountText.text = amountToTask.ToString();

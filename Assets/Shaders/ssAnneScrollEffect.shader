    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                if (i.uv.y > 1.0 / 9.0 && i.uv.y < 4.0 / 9.0)
                {
                    //is the point in the area to be scrolled?
                    float scrollX = shipScrollOffset / 160.0; //divide by 160 to convert to uv space
                    col = tex2D(_MainTex, i.uv + float2(scrollX, 0));
                    if (i.uv.x + scrollX > 1.0) col = tex2D(_MainTex, float2((i.uv.x + scrollX) % 0.1, i.uv.y));
                    // mask the repeating effect on the right side with the left side to get a loop effect
                }
                return col;
            }
            ENDCG
